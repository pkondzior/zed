...
5. (Future) Support **inline decorations/highlights** (selections, squiggles, etc.) without wrapping every range as its own element.
...
pub struct InlineWrapLine {
    ...
    pub logical_start: usize, // min logical index covered by this line
    pub logical_end: usize,   // exclusive max logical index
    // Shaped content for this visual line
    pub shaped_segments: Vec<ShapedSegment>,
    // Geometry
    pub width: Pixels,
    pub height: Pixels,
    pub indent_px: Pixels, // wrap indent already applied to line_origin.x
    pub baseline_above_top: Pixels,
    pub line_origin: Point<Pixels>, // relative to InlineSpan origin
}
...
> The iterator that feeds `TextFragmentCells.offsets` **must** be the exact iterator that powers `LineWrapper` (via `LineFragment::text`) for the same fragment. In the transitional implementation both are `char_indices()` (one scalar == one cell). Once we switch to real `TextCell`s, both systems get switched together. This keeps every wrap `Boundary.ix` aligned with a cell edge so later mapping can assume byte and cell slices stay in lockstep.
...
Other invalidation triggers (regardless of wrap width):

- `InlineContent.fragments` mutated (text added/removed, element builder replaced).
- Any text fragment’s contents or runs change (styles, fonts, attributes).
- `default_font`, `default_font_size`, or the computed `line_height` change.

Every builder/setter that mutates those values must either rebuild a fresh `InlineSpan`
or set `layout_state.dirty = true` immediately so `rewrap_and_shape` runs on the next layout pass.

Inside `rewrap_and_shape`, clear cached vectors up front to avoid accidentally
reading stale slices if an intermediate panic occurs:

```rust
fn rewrap_and_shape(&mut self, cx: &mut Context<InlineSpan>, wrap_width: Pixels) {
    self.layout_state.lines.clear();
    self.layout_state.fragment_layouts.clear();
    self.layout_state.dirty = false;
    // ...rebuild fragments, wrap, shape...
}
```
...
for (frag_index, frag) in content.fragments.iter().enumerate() {
    match frag {
        InlineFragment::Text { text, .. } => {
            ...
            //
            // IMPORTANT: whatever iterator feeds this offsets list is the same
            // iterator that LineWrapper sees. That keeps byte boundaries
            // returned by Boundary.ix perfectly aligned with cell edges.
            //
        ...
        InlineFragment::Element { .. } => {
            ...
            // LineWrapper expects len_utf8 to be a byte count; we can use 1
...
`next_indent` is carried forward to the **next** visual line. During line construction we:

1. Convert `next_indent` (spaces) into pixels using the current text style’s advance for a single space (or a cached indent width).
2. Store that as `indent_px` inside the following `InlineWrapLine`.
3. When computing `line.line_origin.x`, add `indent_px` so every subsequent shaping/painting/hit-test sees the shifted origin.

Downstream consumers must compensate for the shift:

- `InlineWrapLine::index_for_x(x)` subtracts `indent_px` before walking shaped segments.
- Painting starts from `line.line_origin.x` (already includes the indent) rather than resetting to 0.
...
fn compute_logical_slices_for_line(
    ...
    // This must use the same "synthetic bytes" convention as build_line_fragments:
    // - text: real byte length (text.as_ref().len())
    // - element: 1 synthetic byte
    ...
    for range in ranges {
        ...
        let frag_byte_len = match frag {
            ...
            InlineFragment::Element { .. } => 1, // 1 synthetic byte, same as LineFragment::element(width, 1)
        };
        ...
        if intersect_start < intersect_end {
            match frag {
                InlineFragment::Text { .. } => {
                    ...
                    // Invariant (final implementation): boundaries must land on cell edges.
                    // When TextFragmentCells + LineWrapper share the same iterator,
                    // swap these to expect()/debug_assert!().
                    // debug_assert!(offsets.binary_search(&local_byte_start).is_ok());
                    // debug_assert!(offsets.binary_search(&local_byte_end).is_ok());

                    // Transitional implementation keeps the tolerant behavior:
                ...
                InlineFragment::Element { .. } => {
                    // Element occupies exactly one logical cell; byte slicing is meaningless.
                    // We only care that [byte_start, byte_end) "touched" the synthetic byte.
                    let logical_start = range.logical_start;
                    let logical_end = range.logical_end; // == logical_start + 1
...
> **Synthetic element bytes:**  
> Every `InlineFragment::Element` pretends to contribute exactly **1 byte** to the concatenated buffer we pass to `LineWrapper`. That byte is never dereferenced inside a real `&str`; the only consumers are:
>
> 1. `LineFragment::element(width, len_utf8 = 1)` when building fragments.
> 2. `compute_logical_slices_for_line`, which just checks whether the line touched that synthetic byte.
>
> If any future code tries to stitch together an actual buffer using those byte offsets, we should add `debug_assert!`s (in dev builds) that panic immediately—the convention is intentionally synthetic and must stay confined to the span layout logic.
...
// Total byte length of the logical line in the "LineWrapper world".
// Must stay in sync with:
//   - build_line_fragments (text.len() / element -> len_utf8 = 1)
//   - compute_logical_slices_for_line (same conventions)
let total_line_len_bytes: usize = content
    .fragments
    .iter()
    .map(|frag| match frag {
        InlineFragment::Text { text, .. } => text.as_ref().len(),
        InlineFragment::Element { .. } => 1, // synthetic byte for element
    })
    .sum();

let mut prev_byte_ix = 0;
let mut pending_indent_spaces = 0; // applies to the *current* line
let space_advance = text_metrics.space_advance(); // Pixels per indent unit
let mut current_y = px(0.0); // running vertical offset as lines stack
...
for boundary in &boundaries {
    ...
    let indent_px = space_advance * pending_indent_spaces as f32;
    let line_logical_start = logical_slices
        .first()
        .map(|slice| slice.logical_start)
        .unwrap_or(0);
    let line_logical_end = logical_slices
        .last()
        .map(|slice| slice.logical_end)
        .unwrap_or(line_logical_start);

    // TODO: shape these slices into ShapedSegments and push an InlineWrapLine.
    // Remember to add indent_px to line_origin.x.
    lines.push(InlineWrapLine {
        logical_slices,
        logical_start: line_logical_start,
        logical_end: line_logical_end,
        shaped_segments: Vec::new(),
        width: px(0.0),
        height: px(0.0),
        indent_px,
        baseline_above_top: px(0.0),
        line_origin: Point::new(indent_px, current_y),
    });

    pending_indent_spaces = boundary.next_indent as usize;
...
if !trailing_slices.is_empty() {
    lines.push(/* same as above, using pending_indent_spaces */);
}
```
...
Keep each line’s `logical_start`, `logical_end`, and `indent_px` from the earlier mapping step so the shaped line knows its span-level coverage and indent shift.

3. Compute width and baseline as before:

```rust
let mut x = px(0.0); // indent already applied via line.line_origin.x
...
for line in &mut layout_state.lines {
    ...
    // line.line_origin.x already encodes indent_px; leave it alone.
...
- `x_for_index(logical_index)`:
  ...
  3. Walk `shaped_segments` in order, maintaining `x`:
     ...
     - For `ShapedSegment::Element`:
       - If `idx` falls into that element’s `logical_*` range (size == 1 cell), return:
         - `x` (left edge) or `x + width` depending on `CaretAffinity::Upstream/Downstream`.
- `index_for_x(x)`:
  0. Subtract `indent_px` from `x` to get the local coordinate used for shaping.
  ...
  3. For elements: if `x` falls into element box, return `logical_start` or `logical_end`
     based on the same affinity as above (<= center → upstream, else downstream).

Element fragments therefore offer exactly two caret stops (before/after). By keeping
the `logical_*` range `[N, N + 1)` we can still attach higher-level semantics (e.g.
keyboard focus) once the caret lands on the upstream/downstream side of that range.
...
Span-level helpers (e.g. `global_x_y_for_logical_index`) can now just binary-search
`InlineLayoutState.lines` because each `InlineWrapLine` stores `logical_start` and
`logical_end`. No need to rescan line slices—`line.logical_start <= idx < line.logical_end`
pinpoints the line, after which the per-line `x_for_index` logic handles the rest.
